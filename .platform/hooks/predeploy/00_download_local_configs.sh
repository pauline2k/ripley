#!/bin/bash
PYTHONPATH='' aws s3 cp s3://ripley-deploy-configs/${EB_ENVIRONMENT}.py config/production-local.py
...
PYTHONPATH='' aws s3 cp s3://ripley-deploy-configs/ripley/${EB_ENVIRONMENT}_rsa config/lti_rsa
